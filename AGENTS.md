...
3. **Tự động mapping & phân chia (Auto-Assignment):** Hệ thống chạy thuật toán đối chiếu (matching) giữa yêu cầu của task và năng lực/thời gian rảnh của từng thành viên, đề xuất phương án phân công tối ưu.
   - Quản lý có thể xem trước và **điều chỉnh thủ công** kết quả gợi ý trước khi chốt (không tự động hoàn toàn, tránh sai sót).
4. **Gửi thông báo hàng loạt (Cải tiến):** Quản lý bấm 1 nút để gửi email/thông báo phân công tới toàn bộ nhân viên liên quan.
5. **Nhân viên cập nhật tiến độ (Cải tiến, nếu kịp):** Nhân viên đăng nhập, xem task được giao, đánh dấu hoàn thành → hệ thống tự động cập nhật trạng thái & (có thể) đưa dữ liệu này vào lần mapping tiếp theo.
...
## 5. Quy tắc bắt buộc (Mandatory Rules)
