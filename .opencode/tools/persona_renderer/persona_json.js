...
async function runPersonaJson(dataPath) {
    const data = JSON.parse(fs.readFileSync(dataPath, 'utf8'));
    
    // 1. Validate
    const schema = JSON.parse(fs.readFileSync('../../../templates/persona.schema.json', 'utf8'));
    const ajv = new Ajv();
    ...
    // 2. Render
    const template = fs.readFileSync('../../../templates/persona.template.html', 'utf8');
...
runPersonaJson(args[0]).catch(err => {
